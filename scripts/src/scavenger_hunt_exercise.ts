...
const main = async () => {
  ...
  tx.moveCall({
    ...
    arguments: [keyResult, tx.pure.u64(745223)], // The vault code we found!
  });
  ...
  const withdrawResult = tx.moveCall({
    ...
    arguments: [tx.object(VAULT_ID), keyResult],
   ...
   */
  tx.transferObjects([withdrawResult], keypair.getPublicKey().toSuiAddress());
  ...
  console.log("Scavenger hunt result:", result);
  console.log(`View your account: https://suiscan.xyz/testnet/account/${keypair.getPublicKey().toSuiAddress()}`);

  /**
   * Task 7: Run the script with the command below and ensure it works!
   * 
   * npm run scavenger-hunt
   * 
